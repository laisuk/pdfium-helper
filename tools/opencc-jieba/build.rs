...
fn main() {
    #[cfg(target_os = "windows")]
    {
        ...
        let name = env::var("CARGO_PKG_NAME").unwrap_or_else(|_| "opencc-jieba".into());
        ...
        let desc = env::var("CARGO_PKG_DESCRIPTION").unwrap_or_else(|_| {
            "Opencc-Jieba CLI (Simplified/Traditional Chinese Converter)".into()
        });
        ...
        res.set("ProductName", "Opencc-Jieba CLI Tool");
        res.set("CompanyName", &authors);
        res.set("LegalCopyright", "© Laisuk. MIT License");
        res.set("OriginalFilename", "opencc-jieba.exe");
